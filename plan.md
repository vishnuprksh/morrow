...
- AI chat beside the active note
...
- [ ] Establish the application layout:
  ...
  - Collapsible right AI-chat region
...
# Phase 8 — Note-aware AI chat

## Goal

Add a streaming assistant beside the active note.

## Tasks

- [ ] Build a collapsible right-side chat panel.
- [ ] Add `/api/ai/chat` using Vercel AI SDK streaming.
- [ ] Include the active note title and Markdown as bounded context.
- [ ] Display user and assistant messages with loading and stop controls.
- [ ] Keep chat history scoped to the current note or reset it clearly when notes change.
- [ ] Add response actions:
  ...
  - Append to note
  - Create a new note
- [ ] Confirm destructive editor actions before applying them where appropriate.
- [ ] Add context-size handling for long notes through truncation or selected-section context.
- [ ] Keep chat history client-side for the MVP unless persistence is clearly needed.

## Acceptance criteria

- The user can ask a question about the active note and receive a streaming answer.
- Responses can be inserted into the note through explicit actions.
- Switching notes does not silently send context from the previous note.
...
- [ ] Trigger only when:
  ...
  - No selection menu or chat insertion is active
...
7. Ask chat about the note and insert the response.
8. Accept and dismiss autocomplete suggestions.
...
| M4: AI assistant | 8 | User can chat with and insert content into the active note |
...
- [ ] Chat uses only the intended active-note context.
