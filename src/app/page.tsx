  ...
  return (
    <main className="workspace-loading" role="status" aria-live="polite">
      <div className="workspace-loading-card">
        ...
        <p className="workspace-loading-brand">Morrow</p>
...
export default function Home() {
  ...
  async function loadWorkspace(supabase: ReturnType<typeof createClient>) {
    ...
    if (folderResult.error || noteResult.error) {
      setError(
        `Could not load your workspace: ${folderResult.error?.message ?? noteResult.error?.message ?? 'Unknown error'}`,
      );
      setLoading(false);
      return;
    }
    ...
    setNotes(nextNotes);
    setOpenFolders(
      Object.fromEntries(nextFolders.map((folder) => [folder.id, true])),
    );
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.setTimeout(() => {
        setError(
          'Could not connect to authentication. Check your Supabase configuration.',
        );
        setLoading(false);
      }, 0);
      return;
    ...
    autosaveRef.current = createAutosaveController(
      async (noteId, draft, expectedVersion): Promise<SaveResult> => {
        const { data, error: updateError } = await supabase
          .from('notes')
          .update({ ...draft, version: expectedVersion + 1 })
          .eq('id', noteId)
          .eq('version', expectedVersion)
          .select('version, updated_at')
          .maybeSingle();
        if (updateError)
          return { status: 'error', error: new Error(updateError.message) };
      ...
      ({ noteId, ...result }) => {
        if (result.status === 'saved') {
          ...
          setSaveStatus('error');
          setError(
            'This note changed in another tab. Your local draft is preserved for recovery.',
          );
        } else {
          setSaveStatus('error');
  ...
  useEffect(() => {
    ...
    if (previousNote && previousNote !== selectedNote)
  ...
  const [pendingProposal, setPendingProposal] =
    useState<NoteChangeProposal | null>(null);
    ...
    () =>
      notes
        .filter((note) => {
          ...
          if (noteView === 'folder')
            return !note.is_archived && note.folder_id === selectedFolder;
  ...
  function updateNote(
    ...
    setNotes((current) =>
      current.map((item) =>
        item.id === noteId ? { ...item, ...changes } : item,
      ),
    );
  ...
  async function toggleFavorite(note: NoteRow) {
    ...
    setNotes((current) =>
      current.map((item) =>
        item.id === note.id ? { ...item, is_favorite: nextValue } : item,
      ),
    );
    const { error: updateError } = await createClient()
      .from('notes')
  ...
  async function moveNote(note: NoteRow, folderId: string | null) {
    ...
    setNotes((current) =>
      current.map((item) =>
        item.id === note.id ? { ...item, folder_id: folderId } : item,
      ),
    );
  }
  ...
  async function renameFolder(folder: FolderRow) {
    ...
    if (!name) {
      setError('Folder names cannot be blank.');
      return;
    }
    if (name === folder.name) {
  ...
  async function moveToTrash(note: NoteRow) {
    ...
    setNotes((current) =>
      current.map((item) =>
        item.id === note.id ? { ...item, deleted_at: deletedAt } : item,
      ),
    );
    setSelectedNote((current) =>
      current === note.id
        ? (notes.find((item) => item.id !== note.id && item.deleted_at === null)
            ?.id ?? null)
        : current,
    );
    setSelectedNoteIds((current) => {
  ...
  async function restoreNote(note: NoteRow) {
    ...
    setNotes((current) =>
      current.map((item) =>
        item.id === note.id ? { ...item, deleted_at: null } : item,
      ),
    );
    setContextMenu(null);
  }
  ...
  async function bulkMoveToTrash() {
    const ids = [...selectedNoteIds].filter((id) =>
      visibleNotes.some((note) => note.id === id && note.deleted_at === null),
    );
    if (!ids.length || !window.confirm(`Move ${ids.length} notes to Trash?`))
      return;
    const deletedAt = new Date().toISOString();
    const { error: updateError } = await createClient()
      .from('notes')
    ...
    setNotes((current) =>
      current.map((note) =>
        ids.includes(note.id) ? { ...note, deleted_at: deletedAt } : note,
      ),
    );
    setSelectedNoteIds(new Set());
    setSelectingNotes(false);
    setContextMenu(null);
    if (selectedNote && ids.includes(selectedNote))
      setSelectedNote(
        notes.find((note) => !ids.includes(note.id) && note.deleted_at === null)
          ?.id ?? null,
      );
  }
  async function deleteFolder(folder: FolderRow) {
    ...
    const noteCount = notes.filter(
      (note) => note.folder_id && folderIds.includes(note.folder_id),
    ).length;
    if (
      !window.confirm(
        `Delete “${folder.name}”? This deletes ${noteCount} note${noteCount === 1 ? '' : 's'} in this folder and any subfolders.`,
      )
    )
      return;
    const supabase = createClient();
    ...
    setFolders((current) =>
      current.filter((item) => !folderIds.includes(item.id)),
    );
    setNotes((current) =>
      current.filter(
        (note) => !note.folder_id || !folderIds.includes(note.folder_id),
    ...
    setSelectedNote((current) =>
      ...
      notes.some(
        (note) =>
          note.id === current &&
          note.folder_id &&
          folderIds.includes(note.folder_id),
      )
        ? null
        : current,
    );
  ...
  async function uploadImage(file: File) {
    ...
    if (
      ...
      setError(
        'Images must be PNG, JPEG, GIF, WebP or SVG and smaller than 5 MB.',
      );
      return null;
    }
    ...
    const { error: uploadError } = await createClient()
      ...
      .upload(`${user.id}/${selected.id}/${filename}`, file, {
  ...
  function selectVault(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (files) {
      setVaultImported(false);
      setVaultPreview(parseVaultFiles(files));
    }
    event.target.value = '';
  }
  ...
  async function importVault() {
    ...
    setVaultProgress({
      completed: 0,
      total: vaultPreview.notes.length + vaultPreview.images.length,
      current: 'Preparing your vault…',
    });
    try {
      const supabase = createClient();
      ...
      const orderedFolders = [
        ...new Set(
          vaultPreview.notes.flatMap((note) =>
            [vaultPreview.vaultName, ...note.folderPath].map((_, index, path) =>
      ...
      for (const path of orderedFolders) {
        ...
        const { data, error: folderError } = await supabase
          ...
          .insert({
            ...
            parent_id: parentPath ? (folderIds.get(parentPath) ?? null) : null,
            position: folders.length + folderIds.size,
          })
          .select('id')
          .single();
        if (folderError || !data)
          throw new Error(
            folderError?.message ?? 'Could not create import folder',
          );
      ...
      for (const note of vaultPreview.notes) {
        ...
        const { data: importedNote, error: createError } = await supabase
          .from('notes')
          .insert({
            user_id: user.id,
            title: note.title,
            folder_id: folderIds.get(folderPath) ?? null,
            content_markdown: '',
          })
          .select('id')
          .single();
        if (createError || !importedNote)
          throw new Error(
            createError?.message ?? 'Could not create imported note',
          );
        ...
        for (const reference of imageReferences(markdown)) {
          const image =
            ...
            images.get(reference.split('/').pop() ?? reference);
          if (!image) continue;
          const filename = `${crypto.randomUUID()}-${safeFilename(image.file.name, 'image')}`;
          const contentType = imageContentType(image.file);
          if (!contentType)
            throw new Error(`Unsupported image type for ${image.path}`);
          const { error: uploadError } = await supabase.storage
            .from('attachments')
            .upload(`${user.id}/${importedNote.id}/${filename}`, image.file, {
              contentType,
            });
          if (uploadError)
            throw new Error(
              `Could not upload ${image.path}: ${uploadError.message}`,
            );
          uploaded.set(
            reference,
            `/api/attachments/${importedNote.id}/${encodeURIComponent(filename)}`,
          );
          setVaultProgress((current) =>
            current
              ? {
                  ...current,
        ...
        const content = rewriteImageLinks(
          markdown,
          (reference) =>
            uploaded.get(reference) ??
            uploaded.get(reference.split('/').pop() ?? reference),
        );
        const { data, error: noteError } = await supabase
          ...
          .single();
        if (noteError || !data)
          throw new Error(noteError?.message ?? 'Could not import note');
        setNotes((current) => [data, ...current]);
        setVaultProgress((current) =>
          current
            ? {
  ...
  return (
    <main className={`app-shell ${chatOpen ? 'chat-open' : 'chat-closed'}`}>
      <aside className="sidebar">
        ...
        <label className="import-vault-button">
        ...
        <nav className="nav-list">
          <button
            className={`nav-item ${noteView === 'all' ? 'selected' : ''}`}
            onClick={() => {
              setSelectedFolder(null);
              setNoteView('all');
            }}
          >
            ...
            <span>
              {
                notes.filter((note) => !note.deleted_at && !note.is_archived)
                  .length
              }
            </span>
          </button>
          <button
            className={`nav-item ${noteView === 'favorites' ? 'selected' : ''}`}
            onClick={() => {
              setSelectedFolder(null);
            ...
            <span>
              {
                notes.filter(
                  (note) =>
                    !note.deleted_at && note.is_favorite && !note.is_archived,
                ).length
              }
            </span>
          </button>
          <button
            className={`nav-item ${noteView === 'archive' ? 'selected' : ''}`}
            onClick={() => {
              setSelectedFolder(null);
              setNoteView('archive');
            }}
          ...
          <button
            className={`nav-item ${noteView === 'trash' ? 'selected' : ''}`}
            onClick={() => {
              setSelectedFolder(null);
              setNoteView('trash');
            }}
          >
            ...
            <span>{notes.filter((note) => note.deleted_at).length}</span>
          </button>
        </nav>
      ...
      <section className="notes-panel">
        <div className="panel-header">
          ...
          <div className="panel-header-actions">
            <button
              className="icon-button"
              aria-label={
                notesMenuOpen ? 'Close notes actions' : 'Open notes actions'
              }
            ...
            {notesMenuOpen && (
              ...
              >
                <button
                  ...
                  role="menuitem"
                  aria-label={
                    selectingNotes ? 'Cancel selection' : 'Select notes'
                  }
                  onClick={toggleSelectNotes}
                >
                  {selectingNotes ? 'Cancel selection' : 'Select notes'}
                </button>
              </div>
        ...
        {selectingNotes && (
          <div className="bulk-toolbar">
            ...
            <button
              type="button"
              title={visibleNotes.length > 0 && visibleNotes.every((note) => selectedNoteIds.has(note.id)) ? 'Clear all notes' : 'Select all notes'}
              aria-label={visibleNotes.length > 0 && visibleNotes.every((note) => selectedNoteIds.has(note.id)) ? 'Clear all' : 'Select all notes'}
              onClick={toggleSelectAll}
              disabled={visibleNotes.length === 0}
            >
              <CheckCheck size={14} />{' '}
              {visibleNotes.length > 0 &&
              visibleNotes.every((note) => selectedNoteIds.has(note.id))
                ? 'Clear all'
                : 'Select all notes'}
            </button>
        ...
        {loading ? (
          <p className="workspace-message">Loading your notes…</p>
        ) : (
          <div className="note-list">
            {visibleNotes.map((note) => (
              <div
                className={`note-card ${note.id === selectedNote ? 'active' : ''} ${selectingNotes ? 'selection-enabled' : ''}`}
                key={note.id}
              >
                {selectingNotes && (
            ...
            {visibleNotes.length === 0 && (
              <p className="workspace-message">No notes here yet.</p>
            )}
          </div>
        )}
        {noteView !== 'trash' && (
        ...
        {contextMenu && (
          <NoteContextMenu
            ...
            folders={folders}
            onRename={() => {
              setSelectedNote(contextMenu.note.id);
              setContextMenu(null);
            ...
            onArchive={() => void archiveNote(contextMenu.note)}
            onDelete={() => void moveToTrash(contextMenu.note)}
            onRestore={() => void restoreNote(contextMenu.note)}
            onPermanentDelete={() =>
              void permanentlyDeleteNote(contextMenu.note)
            }
            onMove={(folderId) => void moveNote(contextMenu.note, folderId)}
      ...
      <section className="editor">
        <header className="editor-header">
          <div className="breadcrumbs">
            <span>{folderName(selected?.folder_id ?? null)}</span>
            <span>/</span>
          ...
          <div className="editor-tools">
            ...
            <div className="view-toggle" role="group" aria-label="Editor view">
              <button
                type="button"
                className={!rawMarkdown ? 'selected' : ''}
            ...
            <button
              className="icon-button"
              disabled={!selected}
              onClick={() => selected && deleteNote(selected)}
              aria-label="Delete note"
            >
              <Trash2 size={16} />
            </button>
            <button
              className={`icon-button ${selected?.is_favorite ? 'favorite-active' : ''}`}
              type="button"
              disabled={!selected}
              aria-label={
              ...
              aria-pressed={selected?.is_favorite ?? false}
              onClick={() => selected && void toggleFavorite(selected)}
            >
              <Star size={17} />
            </button>
            <button
          ...
          {selected ? (
            <>
              ...
              <input
                ...
                onChange={(event) =>
                  updateNote(selected.id, { title: event.target.value })
                }
                aria-label="Note title"
              />
              {rawMarkdown ? (
                <RawMarkdownEditor
                  value={selected.content_markdown}
                  onChange={(content_markdown) =>
                    updateNote(selected.id, { content_markdown })
                  }
                />
              ) : (
                <MarkdownEditor
                  ...
                  proposal={
                    pendingProposal?.noteId === selected.id
                      ? pendingProposal
                      : null
                  }
      ...
      <div
        ...
        onMouseDown={(event) => {
          ...
          const move = (moveEvent: MouseEvent) => {
            ...
            localStorage.setItem('morrow-agent-width', String(width));
          };
          const stop = () => {
      ...
      {chatOpen && (
        <AgentPanel
          activeNote={selected}
          onProposal={(proposal) => {
            if (!pendingProposal) setPendingProposal(proposal);
          }}
...
function RawMarkdownEditor({
  ...
  return (
    <textarea
      ...
      aria-label="Raw Markdown note content"
      spellCheck={false}
    />
  );
}
...
function RecoveryNotice({
  ...
  note: NoteRow;
  onRecover: (draft: NoteDraft) => void;
}) {
  const recovery = readRecoveryCopy(note.id);
  if (
    !recovery ||
    (recovery.title === note.title &&
      recovery.content_markdown === note.content_markdown)
  )
    return null;
  return (
...
function NoteContextMenu({
  ...
  menu: { note: NoteRow; x: number; y: number };
  folders: FolderRow[];
  onRename: () => void;
  onArchive: () => void;
  onDelete: () => void;
  ...
  return (
    <>
      ...
      >
        {menu.note.deleted_at ? (
          <>
            <button role="menuitem" onClick={onRestore}>
          ...
          <>
            ...
            <label className="move-note-item">
              <Folder size={14} /> Move to{' '}
              <select
                aria-label={`Move ${menu.note.title} to`}
                value={menu.note.folder_id ?? ''}
            ...
            <button role="menuitem" onClick={onArchive}>
              {menu.note.is_archived ? (
                <ArchiveRestore size={14} />
              ) : (
                <Archive size={14} />
              )}{' '}
              {menu.note.is_archived ? 'Unarchive' : 'Archive'}
            </button>
            <button
              className="danger-action"
              role="menuitem"
...
function VaultPreview({
  ...
  preview: { notes: VaultNote[]; images: VaultImage[]; ignored: string[] };
  imported: boolean;
  ...
  if (imported)
    return (
      ...
      >
        <section className="vault-card vault-card-success">
          <header className="vault-header">
            <div>
              <p className="eyebrow">Import complete</p>
              <h2 id="vault-preview-title">Vault imported</h2>
              <p>Your notes and images are ready in your workspace.</p>
            </div>
            <button
              className="icon-button"
              onClick={onClose}
  ...
  return (
    <div
      ...
      aria-labelledby="vault-preview-title"
    >
      <section className={`vault-card ${busy ? 'vault-card-busy' : ''}`}>
        <header className="vault-header">
          <div>
            <p className="eyebrow">Import preview</p>
            <h2 id="vault-preview-title">Review vault import</h2>
            <p>
              {preview.notes.length} notes · {preview.images.length} images
        ...
        {busy && progress && (
          <div className="vault-progress" role="status" aria-live="polite">
            <div className="vault-progress-heading">
              <span>
                <span className="vault-spinner" aria-hidden="true" /> Uploading
                your vault…
              </span>
              <strong>
            ...
            <small>Currently processing {progress.current}</small>
          </div>
        )}
        <div className="vault-summary">
          <div>
            <FileText size={15} />
            <strong>Notes</strong>
            <span>{preview.notes.length} Markdown files</span>
          </div>
          <div>
            <ImagePlus size={15} />
            <strong>Images</strong>
            <span>
              {preview.images.length} image files uploaded to cloud storage
            </span>
          </div>
        </div>
        <div className="vault-file-list">
          <h3>{busy ? 'Importing files' : 'Files to import'}</h3>
          {[
            ...preview.notes.map((note) => `/${note.path} · note`),
            ...preview.images.map((image) => `/${image.path} · image`),
          ]
            .slice(0, 100)
            .map((path) => (
              <code key={path}>{path}</code>
            ))}
          {preview.notes.length + preview.images.length > 100 && (
            <small>Showing the first 100 files.</small>
          )}
        </div>
        {preview.ignored.length > 0 && (
          <p className="vault-ignored">
            Other file types will be ignored ({preview.ignored.length}).
          </p>
        )}
        <footer className="vault-actions">
