...
export async function POST(request: Request) {
  ...
  const { data: credential } = await supabase.from('ai_credentials').select('provider, model, encrypted_api_key').eq('user_id', user.id).maybeSingle();
  if (!credential || credential.provider !== 'openrouter') return NextResponse.json({ error: 'Configure an OpenRouter provider before using the agent.' }, { status: 400 });
  ...
  try {
    const model = createOpenAI({ apiKey: decryptApiKey(credential.encrypted_api_key), baseURL: 'https://openrouter.ai/api/v1' })(credential.model);
    ...
    const result = streamText({
      ...
      tools: {
        ...
        update_note: tool({ description: 'Propose updating a note.', inputSchema: z.object({ id: noteId, title: z.string().max(200).optional(), content_markdown: z.string().max(MAX_NOTE_CHARS).optional(), expectedVersion: z.number().int().positive() }), execute: async (input) => ({ type: 'proposal', tool: 'update_note', input, requiresConfirmation: true }) }),
